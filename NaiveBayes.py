...
class NaiveBayes:
    ...
    def getVectors(self):
        ...
        test_valid_vect = self.vectorizer.transform(self.train + self.validation)
        return train_vect, valid_vect, test_vect, test_valid_vect

    def predictLanguage(self):
        self.trainModel()
        train_vect, valid_vect, test_vect, test_valid_vect = self.getVectors()
        self.model.fit(test_valid_vect, self.train_validation_labels) # refit the model for a bigger input
    ...
    def seeScore(self):
        ...
        self.model.fit(train_vect, self.training_labels) # train the
...
#     nb = None
#     if (option == 1):
#         nb = NaiveBayes(model, tfidf)
#     else:
#         nb = NaiveBayes(model, countVect)
#
#
#     nb.seeScore()
